class apb_seq_master_test extends apb_seq_base_master;
    `uvm_object_utils(apb_seq_master_test)

    function new(string name ="apb_seq_master_test");
        super.new(name);
    endfunction
    ...
    virtual task body();
		...
		repeat(10) begin
        	...
        	start_item(item_seq_m);
       		assert(item_seq_m.randomize() with {pwrite == 1'b1;});
