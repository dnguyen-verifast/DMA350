`ifndef AXI5_SLAVE_DRIVER_BFM_INCLUDED_
`define AXI5_SLAVE_DRIVER_BFM_INCLUDED_

//--------------------------------------------------------------------------------------------
//Interface : axi5_slave_driver_bfm
//Used as the HDL driver for axi5
//It connects with the HVL driver_proxy for driving the stimulus
//--------------------------------------------------------------------------------------------
import axi5_globals_pkg::*;
interface axi5_slave_driver_bfm(input                     aclk    , 
                                input                     aresetn ,
  ...
  //-------------------------------------------------------
  // Importing axi5 slave driver proxy
  //-------------------------------------------------------
  import axi5_slave_pkg::axi5_slave_driver_proxy;

  //Variable : axi5_slave_driver_proxy_h
  ...
  initial begin
    `uvm_info("axi5 slave driver bfm",$sformatf("AXI5 SLAVE DRIVER BFM"),UVM_LOW);
  end

  string name = "AXI5_SLAVE_DRIVER_BFM";
  ...
  task axi5_write_address_phase(inout axi5_write_transfer_char_s data_write_packet);
    @(posedge aclk);
    ...
    if(axi5_slave_drv_proxy_h.axi5_slave_write_addr_fifo_h.is_full()) begin
   ...
   data_write_packet.awburst = mem_wburst [i] ;
    ...
    data_write_packet.awakeup     = awakeup     ;
    data_write_packet.awdomain    = awdomain    ;
    data_write_packet.awinner     = awinner     ;
    data_write_packet.awchid      = awchid      ;
    data_write_packet.awchidvalid = awchidvalid ;
  ...
  endtask: axi5_write_address_phase 

  //-------------------------------------------------------
  // Task: axi5_write_data_phase
  // This task will sample the write data signals
  //-------------------------------------------------------
  task axi5_write_data_phase (inout axi5_write_transfer_char_s data_write_packet, input axi5_transfer_cfg_s cfg_packet);
  ...
  endtask : axi5_write_data_phase

  //-------------------------------------------------------
  // Task: axi5_write_response_phase
  // This task will drive the write response signals
  //-------------------------------------------------------
  
  task axi5_write_response_phase(inout axi5_write_transfer_char_s data_write_packet,
    axi5_transfer_cfg_s struct_cfg,bit[3:0] bid_local);
  ...
  //-------------------------------------------------------
  // Task: axi5_read_address_phase
  // This task will sample the read address signals
  //-------------------------------------------------------
  task axi5_read_address_phase (inout axi5_read_transfer_char_s data_read_packet, input axi5_transfer_cfg_s cfg_packet);
    @(posedge aclk);
    ...
    data_read_packet.ardomain    = ardomain    ;
    data_read_packet.arinner     = arinner     ;
    data_read_packet.archid      = archid      ;
    data_read_packet.archidvalid = archidvalid ;
    data_read_packet.arcmdlink   = arcmdlink   ;
	  j = j+1                                    ;
  ...
  endtask: axi5_read_address_phase
    
  //-------------------------------------------------------
  // Task: axi5_read_data_channel_task
  // This task will drive the read data signals
  //-------------------------------------------------------
  task axi5_read_data_phase (inout axi5_read_transfer_char_s data_read_packet, input axi5_transfer_cfg_s cfg_packet,response_mode_e out_of_order_enable);
   static int j1= 0;
    @(posedge aclk);
    if(out_of_order_enable == RESP_IN_ORDER || out_of_order_enable == ONLY_WRITE_RESP_OUT_OF_ORDER) begin
      ...
      for(int i1=0, k1=0; i1<mem_rlen[j1] + 1; i1++) begin
        ...
        rpoison<=data_read_packet.rpoison[i1];
  ...
  endtask : axi5_read_data_phase

endinterface : axi5_slave_driver_bfm
