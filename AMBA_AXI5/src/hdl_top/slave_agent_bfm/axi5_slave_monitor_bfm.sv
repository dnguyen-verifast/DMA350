`ifndef AXI5_SLAVE_MONITOR_BFM_INCLUDED_
`define AXI5_SLAVE_MONITOR_BFM_INCLUDED_

//--------------------------------------------------------------------------------------------
//Interface : axi5_slave_monitor_bfm
//Used as the HDL monitor for axi5
//It connects with the HVL monitor_proxy for driving the stimulus
//--------------------------------------------------------------------------------------------
import axi5_globals_pkg::*;
interface axi5_slave_monitor_bfm(input aclk, input aresetn,
  ...
  //-------------------------------------------------------
  // Importing axi5 Global Package slave package
  //-------------------------------------------------------
  import axi5_slave_pkg::axi5_slave_monitor_proxy;

  reg[3:0] i = 0;

  //Variable : axi5_slave_monitor_proxy_h
  //Creating the handle for proxy monitor
  axi5_slave_monitor_proxy axi5_slave_mon_proxy_h;
  
  //Printing axi5 slave monitor bfm
  initial begin
    `uvm_info("axi5 slave monitor bfm",$sformatf("AXI5 SLAVE MONITOR BFM"),UVM_LOW);
  end
  ...
  //-------------------------------------------------------
  // Task: axi5_slave_write_address_sampling
  // Used for sample the write address channel signals
  //-------------------------------------------------------
  task axi5_slave_write_address_sampling(output axi5_write_transfer_char_s req ,input axi5_transfer_cfg_s cfg);

    @(posedge aclk);
    `uvm_info("FROM SLAVE MON BFM",$sformatf("from axi5_slave_write_address_sampling "),UVM_HIGH)

    while(awvalid!==1 || awready!==1)begin
      @(posedge aclk);
      `uvm_info("FROM SLAVE MON BFM",$sformatf("Inside while loop from axi5_slave_write_address_sampling"),UVM_HIGH)
    end    
    
    `uvm_info("FROM SLAVE MON BFM",$sformatf("after while loop from axi5_slave_write_address_sampling "),UVM_HIGH)
    ...
    req.awprot = awprot;
    req.awakeup     = awakeup;
    req.awdomain    = awdomain;
    req.awinner     = awinner;
    req.awchid      = awchid;
    req.awchidvalid = awchidvalid;
    `uvm_info("FROM SLAVE MON BFM",$sformatf("after while loop from axi5_slave_write_address_sampling req=%p ",req),UVM_HIGH)
  endtask

  //-------------------------------------------------------
  // Task: axi5_slave_write_data_sampling
  // Used for sample the write data channel signals
  //-------------------------------------------------------
  task axi5_slave_write_data_sampling(output axi5_write_transfer_char_s req ,input axi5_transfer_cfg_s cfg, output int beat_count);
    static reg[7:0] i = 0;
    beat_count = 0;
  forever begin
 ...
 endtask
  ...
  //-------------------------------------------------------
  // Task: axi5_write_response_sampling
  // Used for sample the write response channel signals
  //-------------------------------------------------------
  task axi5_write_response_sampling(output axi5_write_transfer_char_s req ,input axi5_transfer_cfg_s cfg);
  @(posedge aclk);
  ...
  //-------------------------------------------------------
  // Task: axi5_read_address_sampling
  // Used for sample the read address channel signals
  //-------------------------------------------------------  
  task axi5_read_address_sampling(output axi5_read_transfer_char_s req ,input axi5_transfer_cfg_s cfg);
    ...
    req.aruser   = aruser;
    req.ardomain    = ardomain;
    req.arinner     = arinner;
    req.archid      = archid;
    req.archidvalid = archidvalid;
    req.arcmdlink   = arcmdlink;

    `uvm_info("FROM SLAVE MON BFM READ ADDR",$sformatf("datapacket =%p",req),UVM_HIGH)
  endtask

  //-------------------------------------------------------
  // Task: axi5_read_data_sampling
  // Used for sample the read data channel signals
  //-------------------------------------------------------
  task axi5_read_data_sampling(output axi5_read_transfer_char_s req ,input axi5_transfer_cfg_s cfg, output int beat_read_count);
    static reg[7:0] i = 0;
    beat_read_count = 0;
...
endinterface : axi5_slave_monitor_bfm
`endif
