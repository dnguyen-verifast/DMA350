`ifndef AXI5_GLOBALS_PKG_INCLUDED_
`define AXI5_GLOBALS_PKG_INCLUDED_

//--------------------------------------------------------------------------------------------
// Package: axi5_globals_pkg
// Used for storing enums, parameters and defining the structs
//--------------------------------------------------------------------------------------------
package axi5_globals_pkg;

  //-------------------------------------------------------
  // Parameters used in axi5_avip are given below
  //-------------------------------------------------------
  ...
  //Parameter: ID_WIDTH
  //Used to define the width of the AXI5 request/response ID buses
  parameter int ID_WIDTH = 4;

  //Parameter: CHID_WIDTH
  //Used to define the width of the AXI5 SW configurable channel ID (awchid/archid).
  //A value of 0 removes the channel ID signalling.
  parameter int CHID_WIDTH = 4;

  //Parameter: POISON_WIDTH
  //Used to define the width of the AXI5 read data poison signal (rpoison).
  //Nominally one poison bit per 64-bit chunk of read data.
  parameter int POISON_WIDTH = (DATA_WIDTH > 64) ? (DATA_WIDTH/64) : 1;
  ...
  //-------------------------------------------------------
  // Enums used in axi5_avip are given below
  //-------------------------------------------------------
  ...
  typedef struct {
    ...
    bit                     awakeup;     //Pending AXI5 activity indicator
    bit [1:0]               awdomain;    //Shareability domain of the write transaction
    bit [3:0]               awinner;     //Inner domain cache attributes for writes
    bit [CHID_WIDTH-1:0]    awchid;      //SW configurable channel ID indication
    bit                     awchidvalid; //Validity of the SW configurable channel ID
    bit                     awvalid;
    bit	                    awready;
  ...
  } axi5_write_transfer_char_s; 

  //Struct: axi5_r_transfer_char_s
  ...
  typedef struct {
    ...
    bit               [1:0] ardomain;    //Shareability domain of the read transaction
    bit               [3:0] arinner;     //Inner domain cache attributes for reads
    bit    [CHID_WIDTH-1:0] archid;      //SW configurable channel ID indication
    bit                     archidvalid; //Validity of the SW configurable channel ID
    bit                     arcmdlink;   //Command link read indication
    //Read Data Channel Signals
    bit                         [3:0] rid;
    bit [2**LENGTH:0][DATA_WIDTH-1:0] rdata;
    bit            [2**LENGTH:0][1:0] rresp;
    bit            [2**LENGTH:0][3:0] ruser;
    bit [2**LENGTH:0][POISON_WIDTH-1:0] rpoison; //AXI5 read data poison
    bit                               rlast;
    bit                               rvalid;
    bit                               tx_type; 
    ...
    int                               no_of_wait_states;
  } axi5_read_transfer_char_s;

  //Struct: axi5_cfg_char_s
